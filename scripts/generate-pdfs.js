...
/* ===== Ajustes rápidos ===== */
const SHOW_DATE = false;         // no mostrar la fecha en el header
const ENABLE_WATERMARK = false;  // sin marca de agua
const ENABLE_FRAME = false;      // sin marco
...
const FOOTER_LABEL = { es: "Pág.", en: "Page", fr: "Page" };

/* ===== CSS de reserva si no localizamos el tuyo ===== */
...
async function makePDF(pagePath, outPath, lang) {
  ...
  /* ===== Header negro con logo en blanco ===== */
  ...
  const footerTemplate = `
    ...
    <div class="foot">
      ${FOOTER_LABEL[lang] || FOOTER_LABEL.es} <span class="pageNumber"></span> <span class="sep">/</span> <span class="totalPages"></span>
    </div>
  `;
  ...
  const browser = await puppeteer.launch({
    headless: true, // compat amplio (evita 'new' en entornos antiguos)
  ...
  try {
    ...
    // Reescribir rutas absolutas para file://
    ...
    // (Desactivado) Overlay: watermark + frame
    if (ENABLE_WATERMARK || ENABLE_FRAME) {
      await page.evaluate((logoDataURL, flags) => {
      ...
      }, logoDataURL, { wm: ENABLE_WATERMARK, frame: ENABLE_FRAME });
    }
